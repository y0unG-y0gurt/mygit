...
public class TcpTask2s implements Callable {
    private String mac;
    private int intervaltime;
    private int totaltime;

    public TcpTask2s(String mac, int intervaltime, int totaltime){
        this.mac = mac;
...
//        final int port = 6000;//端口号
        ...
        String content;
        String head = this.mac + "/0/radar_" + this.mac + "/bioradar/1/realtimeData2/";
        String[] radar2s = new String[11];
        OutputStream os = sck.getOutputStream();


        for(int i = 0; i < totaltime/intervaltime; i++){
            radar2s[0] = random.nextInt(2) + ".00/" + (100 + i % 3) + ".00/" +String.format("%.2f", Math.random()*30) + "/" + String.format("%.2f", Math.random()*200) + "/" + String.format("%.2f", Math.random()*10) + "/";
            content = head + radar2s[0];
            for (int j = 1; j < radar2s.length; j++){
                radar2s[j] = String.format("%.2f", Math.random()*10 - 5) + "/" + String.format("%.2f", Math.random()*10 - 5) + "/";
                content = content + radar2s[j];
            }
